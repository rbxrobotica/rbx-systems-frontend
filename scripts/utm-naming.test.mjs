...
const transformed = source.replace("import { browser } from '$app/environment';", 'const browser = false;');
assert.notEqual(transformed, source, 'expected SvelteKit environment import');
const compiled = new Bun.Transpiler({ loader: 'ts', target: 'bun' }).transformSync(transformed);

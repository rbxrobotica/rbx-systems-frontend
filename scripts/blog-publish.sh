#!/usr/bin/env bash
# Publish a blog post (Markdown) to the Contabo Object Storage bucket.
#
# The bucket is PRIVATE: this script only writes the object. The site reads it
# server-side via the Content Gateway (src/lib/server/content/gateway.ts) and
# reflects the change within ~60s (gateway cache TTL). No rebuild needed.
#
# Usage:
#   ./scripts/blog-publish.sh blog-posts/2026-03-25-my-post.md
#   ./scripts/blog-publish.sh --all-locales 2026-03-25-my-post
#   ./scripts/blog-publish.sh --all-locales blog-posts/2026-03-25-my-post.pt-BR.md
set -euo pipefail
...
usage() {
  cat <<'EOF'
Usage:
  ./scripts/blog-publish.sh <path-to-post.md>
  ./scripts/blog-publish.sh --all-locales <slug-or-path>

Examples:
  ./scripts/blog-publish.sh blog-posts/2026-03-25-my-post.md
  ./scripts/blog-publish.sh --all-locales 2026-03-25-my-post
  ./scripts/blog-publish.sh --all-locales blog-posts/2026-03-25-my-post.en.md
EOF
}
...
# Legacy single-locale base file used as pt-BR only when an .en sibling exists
# but no explicit .pt-BR sibling does.
is_legacy_pt_br_base() {
  local file="$1"
  local slug base_dir

  [[ "$file" == blog-posts/*.md ]] || return 1
  [[ "$file" != *.pt-BR.md ]] || return 1
  [[ "$file" != *.en.md ]] || return 1

  base_dir=$(dirname "$file")
  slug=$(basename "$file" .md)

  [[ -f "$base_dir/$slug.en.md" ]] || return 1
  [[ ! -f "$base_dir/$slug.pt-BR.md" ]]
}
...
assert_pt_br_orthography() {
  ...
  if LC_ALL=C grep -Eni "$regex" "$file" >/dev/null; then
    ...
    LC_ALL=C grep -Eni "$regex" "$file" >&2 || true
    exit 1
  fi
}
...
validate_file_before_publish() {
  local file="$1"
  assert_utf8 "$file"
  if [[ "$file" == *.pt-BR.md ]] || is_legacy_pt_br_base "$file"; then
...
publish_file() {
  ...
  echo "Publishing: $file → s3://$BUCKET/blog/posts/$slug.md"
  aws s3 cp "$file" "s3://$BUCKET/blog/posts/$slug.md" \
    --endpoint-url "$ENDPOINT" \
    --content-type "text/plain; charset=utf-8"
  echo "Done. Live at /blog/$public_slug within ~60s (gateway cache TTL)."
}
...
if [[ "$PUBLISH_ALL_LOCALES" == "true" ]]; then
  ...
  CANDIDATES=(
    "blog-posts/${BASE_SLUG}.md"
    "blog-posts/${BASE_SLUG}.pt-BR.md"
    "blog-posts/${BASE_SLUG}.en.md"
  )

<script lang="ts">
  ...
  async function handleSubmit(ev: SubmitEvent) {
    ...
    try {
      ...
      status = 'error';
      trackEvent(FORM_ERROR, { source, offer });
    }
  }
</script>

{#if status === 'success'}
  <div class="success-card" class:compact>
...
{:else}
  <form class="form" class:compact class:pending={status === 'submitting'} onsubmit={handleSubmit}>
    <div class="hairline"></div>
    <div class="corners" aria-hidden="true"></div>
    ...
    <div class="row">
      <div class="field">
        <label for="lead-name">{tr('contact.form.name')} *</label>
        <input
          id="lead-name"
          type="text"
          required
      ...
      <div class="field">
        <label for="lead-email">{tr('contact.form.email')} *</label>
        <input
          id="lead-email"
          type="email"
          required
    ...
    <div class="field">
      <label for="lead-phone">{tr('contact.form.phone')}</label>
      <input
        id="lead-phone"
        type="tel"
        bind:value={phone}
        placeholder={tr('contact.form.phonePlaceholder')}
        disabled={status === 'submitting'}
      />
    </div>
    ...
    {#if !compact}
      <div class="field">
        <label for="lead-message">{tr('contact.form.message')}</label>
        <textarea
          id="lead-message"
          rows="3"
          bind:value={message}
    ...
    <div class="checkbox">
      <input
        ...
        disabled={status === 'submitting'}
      />
      <label for="lead-whatsapp">{tr('contact.form.whatsappOptIn')}</label>
    </div>
    ...
    <AltchaWidget
      ...
      disabled={status === 'submitting'}
      labels={{
        verified: tr('altcha.verified'),
        loading: tr('altcha.loading'),
        error: tr('altcha.error'),
        idle: tr('altcha.idle')
      }}
    />
    ...
    {#if status === 'error'}
      <div class="error" role="alert">
        ...
        <span>{tr('contact.form.error')}</span>
      </div>
    {/if}
    ...
    <button type="submit" class="submit" disabled={status === 'submitting'}>
      {#if status === 'submitting'}
        ...
        <span>{tr('contact.form.submitting')}</span>
      {:else}
        <svg
          viewBox="0 0 24 24"
          fill="none"
        ...
        <span>{tr('contact.form.submit')}</span>
      {/if}
    </button>
  </form>
{/if}

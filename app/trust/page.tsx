...
import { trustSections, TrustSection } from "@/app/data/trust/trustData";
      ...
      <main className="relative z-10 mx-auto max-w-6xl px-4 pb-24 pt-28 sm:px-6 sm:pb-32 sm:pt-32 lg:px-12">
        ...
        >
          {trustSections.map((section) => (
            <TrustCard key={section.id} section={section} />
          ))}
        </motion.div>
      </main>
      <Footer dict={dict} />
...
function TrustCard({ section }: { section: TrustSection }) {
  return (
    <motion.div
    ...
    >
      <div>
        ...
        <p className="mt-1 text-xs font-medium text-primary">{section.subtitle}</p>
      </div>
